...
# ╔═╡ 806ee467-aa3b-4033-84e9-1ac40a8b423c
md"""### Mass Matrix

Utilizes a 6x6 Matrix layout with values related to the block making up rows 1-3 and values making up the pendulum beam making up rows 4-6:

$M =
\begin{bmatrix}
m_1&0&0 &0& 0&0\\
0&m_1&0&0&0&0\\
0&0& \frac{m_1}{12}(w^2+h^2)&0&0&0\\
0&0&0&m_2 &0&0 \\
0&0&0&0&m_2&0\\
0&0&0&0&0&\frac{m_2}{12}L^2
\end{bmatrix}$
"""
...
# ╔═╡ a0295c6b-0868-4c8a-a078-82e5fba6550e
md"""### Constraint Jacobian

Utilizes a 4x6 Matrix layout and looks utilizes the first derivative for each constraint equation using the following parameters,

$[x_1,\ y_1,\ \theta_1,\ x_2,\ y_2,\ \theta_2]$

This results in the following matrx with the rows representing the different consrtraint equations and the columns representing the above parameters:
...
# ╔═╡ 501b6a88-51da-4e05-9696-7408d54397e2
md"""###  External Forces Matrix

This is a summation of all the external forces working on the system which in this case is the spring force on the block as well as gravity on both bodies. This results in the following 6x1 matrix:
...
# ╔═╡ b4bea3da-a296-40cd-9118-8e6044185df5
md"""###  Differential Constraint Matrix

This matrix is the second order derivatives of the constrain equations using the following expression:

$Q_d = -C_{tt} - (C_q\dot{q})_q\dot{q}-2C_{qt})\dot{q}$

Since none of the constraint equations are time dependent the matrix $Q_d$ is only reliant upon the second term of this expression. This results in the following 4x1 matrix:

$Q_d  =
\begin{bmatrix}
0\\
0\\
\frac{L\dot{\theta_2}^2}{2}\cos\theta_2\\
\frac{L\dot{\theta_2}^2}{2}\sin\theta_2
\end{bmatrix}$
"""
...
# ╔═╡ 9658c252-df82-41a2-bdf9-dfd38fd40f10
md"""###  Augmented System

With all of these matrices the system can be set up as so to solve for the resulting positional data of both bodies as well as the constraint reaction forces using an ODE solution method 
...
All relevant data can be found in the following section for data visualization
"""
...
# ╔═╡ 27ae2f1c-9d19-4b24-a00b-ef053124ff80
md"""
The following cells are sanity checks to make sure the beam is consistent in it's length across the operation"""
...
# ╔═╡ 32eb7806-c913-4099-b44d-8692bfcb2280
md"""### Reaction Force Plot Visualizations"""
...
begin
    ...
    # --- Plot colored line ---
	beam_tip_speed = @animate for index in 1:5:length(t)
		plot(
        	beam_tip_x,
        	beam_tip_y,
...
# ╔═╡ 5326e02f-021d-43a8-b2c9-e7490debed8e
gif(beam_tip_speed, "beam_speed_with_point.gif", fps=800)
